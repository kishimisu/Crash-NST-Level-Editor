...
Custom levels are saved as `.pak` archives just like regular levels. They are saved uncompressed by default because it is faster when you test often; use `File → Compress & Save` before sharing. See [Save, play and backups](/level-editor/save-play-and-backups/).

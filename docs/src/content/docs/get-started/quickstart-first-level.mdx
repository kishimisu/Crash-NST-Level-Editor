...
7. **Share it.**
   ...
   Use  `File → Remove unused files`, then `File → Compress & Save` to produce a smaller `.pak` archive, then share that file. Players open it using the **Play Custom Level** button. See [Play, save and publish](/level-editor/play-save-export/).

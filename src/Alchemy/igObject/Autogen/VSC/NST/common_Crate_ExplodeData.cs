namespace Alchemy
{
    [ObjectAttr(nst: 168, ctr: 160, align: 4, metaType: typeof(CVscComponentData))]
    public class common_Crate_ExplodeData : CVscComponentData
    {
        [FieldAttr(nst: 40, ctr: 32)] public igHandleMetaField _Entity_Tag_0x28 = new();
        [FieldAttr(nst: 48, ctr: 40)] public float _Float_0x30;
        [FieldAttr(nst: 52, ctr: 44)] public float _DamageRadius_0x34;
        [FieldAttr(nst: 56, ctr: 48)] public float _DamageRadius_0x38;
        [FieldAttr(nst: 64, ctr: 56)] public igHandleMetaField _Game_Bool_Variable = new();
        [FieldAttr(nst: 72, ctr: 64)] public float _Float_0x48;
        [FieldAttr(nst: 76, ctr: 68)] public float _Float_0x4c;
        [FieldAttr(nst: 80, ctr: 72)] public float _DamageRadius_0x50;
        [FieldAttr(nst: 88, ctr: 80)] public igHandleMetaField _ExplosionSound = new();
        [FieldAttr(nst: 96, ctr: 88)] public igHandleMetaField _Entity_Tag_0x60 = new();
        [FieldAttr(nst: 104, ctr: 96)] public igHandleMetaField _Entity_Tag_0x68 = new();
        [FieldAttr(nst: 112, ctr: 104)] public igHandleMetaField _ExplosionVFX = new();
        [FieldAttr(nst: 120, ctr: 112)] public igHandleMetaField _Query_Filter = new();
        [FieldAttr(nst: 128, ctr: 120)] public igHandleMetaField _Damage = new();
        [FieldAttr(nst: 136, ctr: 128)] public string? _String_0x88 = null;
        [FieldAttr(nst: 144, ctr: 136)] public string? _String_0x90 = null;
        [FieldAttr(nst: 152, ctr: 144)] public string? _String_0x98 = null;
        [FieldAttr(nst: 160, ctr: 152)] public string? _String_0xa0 = null;
    }
}

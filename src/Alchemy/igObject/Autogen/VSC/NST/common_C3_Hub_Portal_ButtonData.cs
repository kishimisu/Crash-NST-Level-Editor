namespace Alchemy
{
    [ObjectAttr(nst: 168, ctr: 160, align: 4, metaType: typeof(CVscComponentData))]
    public class common_C3_Hub_Portal_ButtonData : CVscComponentData
    {
        [FieldAttr(nst: 40, ctr: 32)] public float _Float;
        [FieldAttr(nst: 48, ctr: 40)] public igHandleMetaField _Entity_0x30 = new();
        [FieldAttr(nst: 56, ctr: 48)] public igHandleMetaField _Entity_0x38 = new();
        [FieldAttr(nst: 64, ctr: 56)] public igHandleMetaField _Zone_Info = new();
        [FieldAttr(nst: 72, ctr: 64)] public igHandleMetaField _Vfx_Effect_0x48 = new();
        [FieldAttr(nst: 80, ctr: 72)] public igHandleMetaField _common_C2_WarpRoom_LevelPortalDatas049 = new();
        [FieldAttr(nst: 88, ctr: 80)] public igHandleMetaField _common_C2_WarpRoom_LevelPortalDatas028 = new();
        [FieldAttr(nst: 96, ctr: 88)] public igHandleMetaField _igEntity = new();
        [FieldAttr(nst: 104, ctr: 96)] public int _Int;
        [FieldAttr(nst: 108, ctr: 100)] public EZoneCollectibleType _common_C2_WarpRoom_LevelPortalDatas;
        [FieldAttr(nst: 112, ctr: 104)] public bool _Bool_0x70;
        [FieldAttr(nst: 116, ctr: 108)] public ECrashSecretZones _E_Crash_Secret_Zones;
        [FieldAttr(nst: 120, ctr: 112)] public igHandleMetaField _Vfx_Effect_0x78 = new();
        [FieldAttr(nst: 128, ctr: 120)] public igHandleMetaField _Vfx_Effect_0x80 = new();
        [FieldAttr(nst: 136, ctr: 128)] public igHandleMetaField _Sound_0x88 = new();
        [FieldAttr(nst: 144, ctr: 136)] public igHandleMetaField _Sound_0x90 = new();
        [FieldAttr(nst: 152, ctr: 144)] public igHandleMetaField _Sound_0x98 = new();
        [FieldAttr(nst: 160, ctr: 152)] public bool _Bool_0xa0;
    }
}

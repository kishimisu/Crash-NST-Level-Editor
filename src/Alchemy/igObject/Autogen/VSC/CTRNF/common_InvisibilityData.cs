namespace Alchemy
{
    [ObjectAttr(nst: 120, ctr: 112, align: 4, metaType: typeof(CVscComponentData))]
    public class common_InvisibilityData : CVscComponentData
    {
        [FieldAttr(nst: 40, ctr: 32)] public igHandleMetaField _Vfx_Effect_0x20 = new();
        [FieldAttr(nst: 48, ctr: 40)] public igHandleMetaField _Vfx_Effect_0x28 = new();
        [FieldAttr(nst: 56, ctr: 48)] public igHandleMetaField _Vfx_Effect_0x30 = new();
        [FieldAttr(nst: 64, ctr: 56)] public igHandleMetaField _Vfx_Effect_0x38 = new();
        [FieldAttr(nst: 72, ctr: 64)] public igHandleMetaField _Sound_0x40 = new();
        [FieldAttr(nst: 80, ctr: 72)] public igHandleMetaField _Sound_0x48 = new();
        [FieldAttr(nst: 88, ctr: 80)] public igHandleMetaField _Sound_id_dac6fms2001 = new();
        [FieldAttr(nst: 96, ctr: 88)] public igHandleMetaField _Sound_id_dac6fms2002 = new();
        [FieldAttr(nst: 104, ctr: 96)] public igHandleMetaField _Rumble_Data_0x60 = new();
        [FieldAttr(nst: 112, ctr: 104)] public igHandleMetaField _Rumble_Data_0x68 = new();
    }
}

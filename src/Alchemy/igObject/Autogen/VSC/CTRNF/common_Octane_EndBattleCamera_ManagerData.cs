namespace Alchemy
{
    [ObjectAttr(nst: 80, ctr: 72, align: 4, metaType: typeof(CVscComponentData))]
    public class common_Octane_EndBattleCamera_ManagerData : CVscComponentData
    {
        [FieldAttr(nst: 40, ctr: 32)] public igHandleMetaField _Camera_Base_0x20 = new();
        [FieldAttr(nst: 48, ctr: 40)] public igHandleMetaField _Camera_Base_0x28 = new();
        [FieldAttr(nst: 56, ctr: 48)] public igHandleMetaField _Camera_Base_0x30 = new();
        [FieldAttr(nst: 64, ctr: 56)] public igHandleMetaField _Component_Data = new();
        [FieldAttr(nst: 72, ctr: 64)] public float _Float;
    }
}

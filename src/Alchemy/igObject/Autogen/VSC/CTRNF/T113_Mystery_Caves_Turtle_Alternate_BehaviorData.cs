namespace Alchemy
{
    [ObjectAttr(nst: 136, ctr: 128, align: 4, metaType: typeof(CVscComponentData))]
    public class T113_Mystery_Caves_Turtle_Alternate_BehaviorData : CVscComponentData
    {
        [FieldAttr(nst: 40, ctr: 32)] public bool _Bool;
        [FieldAttr(nst: 48, ctr: 40)] public igHandleMetaField _Entity_0x28 = new();
        [FieldAttr(nst: 56, ctr: 48)] public igHandleMetaField _Entity_0x30 = new();
        [FieldAttr(nst: 64, ctr: 56)] public igHandleMetaField _Entity_0x38 = new();
        [FieldAttr(nst: 72, ctr: 64)] public igHandleMetaField _Vfx_Effect_0x40 = new();
        [FieldAttr(nst: 80, ctr: 72)] public igHandleMetaField _Vfx_Effect_0x48 = new();
        [FieldAttr(nst: 88, ctr: 80)] public igHandleMetaField _Sound = new();
        [FieldAttr(nst: 96, ctr: 88)] public float _Float_0x58;
        [FieldAttr(nst: 100, ctr: 92)] public float _Float_0x5c;
        [FieldAttr(nst: 104, ctr: 96)] public igHandleMetaField _Kart_Boost_Data_0x60 = new();
        [FieldAttr(nst: 112, ctr: 104)] public igHandleMetaField _Kart_Boost_Data_0x68 = new();
        [FieldAttr(nst: 120, ctr: 112)] public igHandleMetaField _Vfx_Effect_0x70 = new();
        [FieldAttr(nst: 128, ctr: 120)] public igHandleMetaField _Vfx_Effect_0x78 = new();
    }
}
